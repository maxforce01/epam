...
public class LoginPageCommandTest extends Mockito {
	...
	public void testDoPost() throws Exception {
		...
		new LoginPageCommand().execute(request, response, ActionType.POST);
	}
	...
	public void testDoGet() throws Exception {
		...
		when(session.getAttribute("user")).thenReturn(new UserDao().findUser((long) 2));
		when(session.getAttribute("userRole")).thenReturn(Role.ADMIN);
		new LoginPageCommand().execute(request, response, ActionType.GET);
	}
