...
public class UserListCommandTest extends Mockito{

	@Test
	public void testDoPost() throws Exception {
		HttpServletRequest request = Mockito.mock(HttpServletRequest.class);
		HttpServletResponse response = Mockito.mock(HttpServletResponse.class);
		assertEquals(Path.COMMAND__ERROR_PAGE+"error.invalid.request", new UserListCommand().execute(request, response, ActionType.POST));
	}
	...
	public void testDoGet() throws Exception {
		...
		assertEquals(Path.LOGIN__PAGE, new UserListCommand().execute(request, response, ActionType.GET));
	}
	...
	public void testDoGet2() throws Exception {
		...
		assertEquals(Path.PAGE__ERROR_PAGE, new UserListCommand().execute(request, response, ActionType.GET));
	}
	...
	public void testDoGet3() throws Exception {
		...
		when(session.getAttribute("user")).thenReturn(new User());
		when(session.getAttribute("userRole")).thenReturn(Role.ADMIN);
		assertEquals(Path.COMMAND_LIST_USERS, new UserListCommand().execute(request, response, ActionType.GET));
	}
