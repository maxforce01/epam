...
public class SortCarCommandTest extends Mockito {
	...
	public void testDoPost() throws Exception {
		...
		assertEquals(Path.COMMAND__ERROR_PAGE + "error.invalid.request",
				new SortCarCommand().execute(request, response, ActionType.POST));
	}
	...
	public void testDoGet() throws Exception {
		...
		when(session.getAttribute("sort")).thenReturn("model");
		assertEquals(Path.COMMAND__LIST_CARS, new SortCarCommand().execute(request, response, ActionType.GET));
	}
	...
	public void testDoGet2() throws Exception {
		...
		when(session.getAttribute("sort")).thenReturn("model");
		assertEquals(Path.LOGIN__PAGE, new SortCarCommand().execute(request, response, ActionType.GET));
	}
	@Test
	public void testDoGet3() throws Exception{
		...
		when(session.getAttribute("sort")).thenReturn("model");
		assertEquals(Path.PAGE__ERROR_PAGE, new SortCarCommand().execute(request, response, ActionType.GET));
	}
	@Test
	public void testDoGet4() throws Exception{
		...
		when(session.getAttribute("user")).thenReturn(new User());
		when(session.getAttribute("userRole")).thenReturn(Role.ADMIN);
		when(session.getAttribute("sort")).thenReturn("asd");
		assertEquals(Path.PAGE__ERROR_PAGE, new SortCarCommand().execute(request, response, ActionType.GET));
	}
}
